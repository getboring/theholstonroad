...
export default function VenuePlaceholder({ name, type = 'major', className = '' }: VenuePlaceholderProps) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    shapes.push(
      <circle key={i} cx={x} cy={y} r={r} fill={colors.accent} opacity={opacity} />
    )
  }
  ...
  return (
    <div className={`relative overflow-hidden ${className}`} style={{ backgroundColor: colors.bg }}>
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice">
        ...
        {pattern === 3 && (
          <>
            <circle cx="50" cy="50" r="35" fill="none" stroke={colors.accent} strokeWidth="0.5" opacity="0.15" />
            <circle cx="50" cy="50" r="25" fill="none" stroke={colors.accent} strokeWidth="0.5" opacity="0.1" />
            <circle cx="50" cy="50" r="15" fill="none" stroke={colors.accent} strokeWidth="0.5" opacity="0.08" />
          </>
        )}
        {shapes}
      </svg>
