...
const DB_PATH =
	"/Users/codyboring/projects/trailhead/.wrangler/state/v3/d1/miniflare-D1DatabaseObject/f03272be288e4dea1c88177a64a04460a0b5f8e9d8892f8e2b9956c57bc67174.sqlite";
...
function seedTenant(
	...
	const venueRows = mod.majorVenues.map((v) => ({
		...v,
		isActive: true,
		createdAt: new Date(now * 1000),
		updatedAt: new Date(now * 1000),
	}));
	const waysideRows = mod.waysides.map((w) => ({
		...w,
		isActive: true,
		createdAt: new Date(now * 1000),
		updatedAt: new Date(now * 1000),
	}));
	const eventRows = [...mod.recurringEvents, ...mod.specialEvents].map((e) => ({
		...e,
		isActive: true,
		createdAt: new Date(now * 1000),
		updatedAt: new Date(now * 1000),
	}));
	const dmoRows = mod.dmoContacts.map((d) => ({
		...d,
		isActive: true,
		createdAt: new Date(now * 1000),
		updatedAt: new Date(now * 1000),
